...
public partial class FileProcesser
{
    ...
    public static bool LoadFromSDZ(string path)
    {
        ...
        if (File.Exists(pathMP3))
        {
            ...
            chart_info_dialog.bgm_path_label.Text = pathMP3;
        }
        string pathWav = Path.GetDirectoryName(path) + "\\music.wav";
        if (File.Exists(pathWav))
        {
            MediaManager.LoadBGMFromPath(pathWav);
            chart_info_dialog.bgm_path_label.Text = pathWav;
        }
        string pathOgg = Path.GetDirectoryName(path) + "\\music.ogg";
        if (File.Exists(pathOgg))
        {
            MediaManager.LoadBGMFromPath(pathOgg);
            chart_info_dialog.bgm_path_label.Text = pathOgg;
        }
        return true;
    }
}

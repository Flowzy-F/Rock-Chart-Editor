...
public partial class Editor : Node
{
    ...
    #region Grid
    public static readonly int TrackCount = 5;
    public static readonly Dictionary<string, int> SPNoteOffset = new Dictionary<string, int>();//BPM Notes' track number=TrackCount-SPNoteOffset["BPM"]
    ...
    public static readonly Vector2 DrawerPosition = new Vector2(25, 75);
    ...
    public List<PanelContainer> NoteEdits = new List<PanelContainer>();
    ...
    public bool PlaceHitNote(NoteType type, Fraction position, int track, int count, int remove_count, bool add_count, float scale = 1f,int y_offset=0)
    {
        HitNoteData d = new HitNoteData(type, position, track, count, remove_count, scale,y_offset);
    ...
    {
        BPMNoteData d = new BPMNoteData(NoteType.BPM, position, TrackCount - SPNoteOffset["BPM"], bpm_value);
        ICommand cmd = new AddBPMNoteCommand([d], rebuild_bpm_time_line);
        return (CommandSystem.ExecuteCommand(cmd));
    }
    public bool PlaceBKGNote(Fraction position)
    {
        NoteData d = new NoteData(NoteType.BKG, position, TrackCount - SPNoteOffset["BKG"]);
        ICommand cmd = new AddBKGNoteCommand([d]);
        return (CommandSystem.ExecuteCommand(cmd));
    }
    /// <summary>
    /// Make a parameter be the default value if you don't want to modify it.
    /// </summary>
    public bool ModifyHitNoteData(List<NoteHash> targets, int count = -1, int remove_count = -1, float scale = -1, NoteType type = NoteType.Invalid,int y_offset=-1)
    {
        ModifyHitNoteDataCommand cmd = new ModifyHitNoteDataCommand(targets, count, remove_count, scale, type,y_offset);
        return (CommandSystem.ExecuteCommand(cmd));
    }
    public bool ModifyBPMNoteData(List<NoteHash> targets, float bpm_value)
    {
        ModifyBPMNoteDataCommand cmd = new ModifyBPMNoteDataCommand(targets, bpm_value);
        return (CommandSystem.ExecuteCommand(cmd));
    }
    /// <summary>
    ...
    /// No BPM Timeline rebuilding.
    /// </summary>
    public bool MoveNoteTo(NoteHash h, float track, Fraction position)
    {
        MoveNoteToCommand cmd = new MoveNoteToCommand(h, position, track);
        return (CommandSystem.ExecuteCommand(cmd));
    }
    ...
    public bool DeleteNote(Fraction position, float track, int count = -1, bool rebuild_bpm_timeline = true)
    {
        NoteHash delete_hash = new NoteHash(position, track);
        DeleteNotesCommand cmd = new DeleteNotesCommand([delete_hash], rebuild_bpm_timeline, count);
        return (CommandSystem.ExecuteCommand(cmd));
    }
    /// <summary>
    ...
    public override void _Process(double delta)
    {
        if (MediaManager.IsPlaying)
        {
            ...
            {
                bar_pos = bar_pos + SyncTimeSystem.ToBarPosition((float)delta);
            }
            else
            {
    ...
    public override void _Ready()
    {
        ...
        #endregion
        #region SPOffset
        SPNoteOffset.Add("BPM", 1);
        SPNoteOffset.Add("BKG", 2);
        #endregion
        #region Drawers
    ...
    void on_load_dialog_file_selected(string path)
    {
        ...
        {
            Editor.Instance.TipManager.AddTip("Error:Faild to load this file!", 0.5f, TipManager.TipIcon.Warning, TipManager.TipColor.Red);
        }
        else
        {
            SavePath = path;
            Editor.Saved = true;
            Editor.Instance.TipManager.AddTip("Successfully Loaded!", 2.0f, TipManager.TipIcon.Information, TipManager.TipColor.Green);
            SyncTimeSystem.BuildBPMTimeLine();
    ...
    void on_save_dialog_file_selected(string path)
    {
        ...
        TipManager.AddTip($"Saved at {Editor.SavePath}!", 1.5f, TipManager.TipIcon.Information, TipManager.TipColor.Green);
    }

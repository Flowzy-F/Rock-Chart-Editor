...
public partial class HitNoteEdit : PanelContainer
{
    ...
    public override void _Ready()
    {
        ...
        type_option.Connect(OptionButton.SignalName.ItemSelected, new Callable(this, nameof(change_type)));
    ...
    public void UpdateLineEdits()
    {
        ...
        foreach (NoteHash h in SelectedNoteList)
        {
            ...
            {
                if (d.Track >= TrackCount - SPNoteOffset.Count)//UNDONE:SPNoteOffsetCount=sp note count?
        ...
        if (pointing_notedatas.All(n => n.Type == pointing_notedatas[0].Type))
        {
            switch (pointing_notedatas[0].Type)
            {
    ...
    void change_scale()
    {
        ...
        {
            if (Editor.Instance.ModifyHitNoteData(SelectedNoteList, -1, -1, scale_r))
    ...
    void change_bar()//Only allowed when selecting a single note.
    {
        ...
        {
            if (
            Editor.Instance.MoveNoteTo(SelectedNoteList[0], SelectedNoteList[0].Track,
                new Utils.Fraction(SelectedNoteList[0].Position.GetTrueNumerator() + bar_r * SelectedNoteList[0].Position.Denominator
                , SelectedNoteList[0].Position.Denominator)))
                Editor.Instance.NoteDrawer.QueueRedraw();
        }
        else
        {
    ...
    void change_count()//Only allowed when selecting a single note.
    {
        ...
        {
            if (Editor.Instance.ModifyHitNoteData(SelectedNoteList, count_r))
                Editor.Instance.NoteDrawer.QueueRedraw();
        }
        else
        {
    ...
    void change_remove_count()//Only allowed when selecting a single note.
    {
        ...
        {
            if (Editor.Instance.ModifyHitNoteData(SelectedNoteList, -1, remove_count_r))
                Editor.Instance.NoteDrawer.QueueRedraw();
        }
        else
        {
    ...
    void change_type(int selected)//Only allowed when selecting a single note.
    {
        NoteType target_type = NoteType.Normal;
        switch (selected)
        {
            case 0:
                target_type = NoteType.Normal;
                break;
            case 1:
                target_type = NoteType.Gold;
                break;
            default:
                break;
        }
        if (Editor.Instance.ModifyHitNoteData(SelectedNoteList, -1, -1, -1, target_type))
            Editor.Instance.NoteDrawer.QueueRedraw();
        switch (pointing_notedatas[0].Type)
        {
            case NoteType.Normal:
    ...
    void change_track()
    {
        ...
        if (float.TryParse(track_edit.Text.Trim(), out float track_r) && track_r >= -0.5f && track_r <= TrackCount - SPNoteOffset.Count-0.5f)
        {
            if (Editor.Instance.MoveNoteTo(SelectedNoteList[0], track_r, SelectedNoteList[0].Position))
                Editor.Instance.NoteDrawer.QueueRedraw();
        }
        else
        {
    ...
    void change_fraction()//Only allowed when selecting a single note.
    {
            ...
            (num_r >= 0 && num_r < den_r && den_r > 0))
        {
            if (
            Editor.Instance.MoveNoteTo(SelectedNoteList[0], SelectedNoteList[0].Track,
                new Utils.Fraction(SelectedNoteList[0].Position.GetWhole() * den_r + num_r, den_r)))
                Editor.Instance.NoteDrawer.QueueRedraw();
        }
        else if (int.TryParse(fraction_edit.Text.Trim(), out int r) && r == 0)
        {
            if (
            Editor.Instance.MoveNoteTo(SelectedNoteList[0], SelectedNoteList[0].Track,
                new Utils.Fraction(SelectedNoteList[0].Position.GetWhole(), SelectedNoteList[0].Position.Numerator)))
                Editor.Instance.NoteDrawer.QueueRedraw();
        }
        else
        {
    ...
    void change_y_offset()
    {
        ...
        {
            if (Editor.Instance.ModifyHitNoteData(SelectedNoteList, -1,-1,-1,NoteType.Invalid,yoffset_r))
                Editor.Instance.NoteDrawer.QueueRedraw();
        }
        else
        {
